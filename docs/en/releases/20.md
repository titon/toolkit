...
The second major release of Toolkit, aptly named Gallant Golem, based on the Jewish mythological creature, 
is jam-packed with new features, major refactors, and extensive changes. This marks the path for Toolkit's bright future.
...
Internet Explorer 8 has also been dropped. If IE8 support is required, please use Toolkit 1.0.

## Improved tooling ##

The entire JavaScript code base has been reorganized for [RequireJS](http://requirejs.org/). All components, extensions, features, etc, 
have been packaged into individual JS files with their own dependencies. This greatly improved maintenance, 
and reduced the file size and custom builds as only required dependencies are now included.
...
To change the class names found within JavaScript templates, you will need to override the `template` manually.

## Automatic prefixing ##

Vendor (non-browser) prefixing is now automatically applied to all CSS class names in the Sass layer and within JavaScript templates. 
...
The major issues we had in 1.0 was components getting confused between the parent and child when binding events 
and detecting child elements. Namespaces solve all issues with the minor inconvenience of requiring a bit of markup.
